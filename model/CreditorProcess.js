...
class CreditorProcess {
    // Select
    getAllRawData() {
        return database.connectAndQuery("select * from public.creditor_set")
    }

    checkownerCompany(ownerCompany) {
        return database.connectAndQuery(`select company,address1,tel,fax from public.creditor_set where id = '${ownerCompany}'`)
    }
    ...
    getCreditorData (payment_type,paymenttype_id,customer_id,customer,selling_type,sellingtype_id) {
...
module.exports = CreditorProcess
